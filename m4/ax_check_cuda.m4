...
if [[[ $USE_CUDA == "yes" && $VALID_CUDA == "yes" ]]] ; then
  ...
  CUDA_LDFLAGS="-L $CUDA_HOME/lib64 -L $CUDA_HOME/lib64/stubs -L $CUDA_SDK_PATH/common/lib -lcuda -lcudart -lcurand"
  ...
  AC_CHECK_LIB([curand], [curandGenerate],, AC_MSG_WARN([Could not find libcurand]); VALID_CUDA=no)
  LIBS=""
