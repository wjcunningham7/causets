...
x0 = xval[i];
\[Lambda]0 = \[Lambda]val[j];
Print@With[{x = x0, \[Lambda] = \[Lambda]0}, t1[[i,j]][x, \[Lambda]]];
...
(* ClearAll[t2f]
t2f = Compile[{{i, _Integer}, {j, _Integer}, {k, _Integer}}, Block[{
	ic = Floor[(i - 1) / gran] + 1,
	jc = Floor[(j - 1) / gran] + 1,
	kc = Floor[(k - 1) / gran] + 1},
	If[i >= j, 0,
...
(* Lookup Table *)
(* t2 = ParallelTable[With[{i = a, j = b, k = c}, t2f[i, j, k]], {a, xcellsf}, {b, xcellsf}, {c, \[Lambda]cellsf}]; *)

(* Use Do Instead *)
t2 = Table[0, {xcellsf}, {xcellsf}, {\[Lambda]cellsf}];
Do[With[{i = a, j = b, k = c}, Block[{
	ic = Floor[(i - 1) / gran] + 1,
	jc = Floor[(j - 1) / gran] + 1,
	kc = Floor[(k - 1) / gran] + 1},
	Print@\[Lambda]valf[k];
